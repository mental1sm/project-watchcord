...
export class GuildController {
  ...
  async findAll(@Param('appId') appId: string, @Query('fetch', new ParseBooleanPipe()) fetch: boolean) {
    ...
    const guildStatistics = await this.guildService.getGuildStatistics(appId);
  ...
  @ApiQuery({ name: 'fetch', description: 'Fetch from Discord API', required: false })
  findOne(@Param('appId') appId: string, @Param('guildId') guildId: string, @Query('fetch', new ParseBooleanPipe()) fetch: boolean) {
    if (fetch === true) {
      return this.guildService.fetch(guildId);
    }
    return this.guildService.findOne(appId, guildId);
  }
}
