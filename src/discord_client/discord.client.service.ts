...
export class DiscordClientService {
     ...
     */
    async fetchMessages(channelId: string, options: MessageFetchingOptions): Promise<AxiosResponse<Message[]>> {
     ...
     */
    async fetchMessage(channelId: string, messageId: string): Promise<AxiosResponse<Message>> {
        return this.get(`/channels/${channelId}/messages/${messageId}`)
    }

    private parseMessageFetchingOptions(options: MessageFetchingOptions): URLSearchParams {
        const queryOptions = new URLSearchParams();
        if (options.limit !== undefined) queryOptions.append('limit', options.limit.toString());
        if (options.after !== undefined) queryOptions.append('after', options.after);
        if (options.before !== undefined) queryOptions.append('before', options.before);
        if (options.around !== undefined) queryOptions.append('around', options.around);
        return queryOptions;
    }
}